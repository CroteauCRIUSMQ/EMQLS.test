Mqls.test2 <-
function(data,lstk,fam.id,fl,k1,k){
	# The Mqls test
	# data : the database 
    ...
    if(k1==0){print(k1)
      stop(paste(paste("k1=",k1,":",sep=""),"This argume can't take value 0"))
      }else{  
    ...
    # the function Mqls
      fMqls<-Vectorize(function(k){
       ...
       # The optimize the function Mqls
         M1<-optimize(fMqls,interval=c(0,1),lower=0,upper=1,maximum=TRUE)$objective
       
       # To compute the integrate of the function
         V<-Intg_Rim(sqfct,0,1)
       
       # the corrected p-value
         Pco1=(1-pchisq(M1,1))+V*exp(-(1/2)*M1)*(2*pi)^(-1/2)
       
       # the prevalace
         rr<-ifelse(model.matrix(fl,data=data)[,2]>0,1,0)
         k0<-sum(rr)/length(rr)
         
       if(missing(k)==TRUE){vdev<-c(round(fMqls(k0),digits=4),1,round((1-pchisq(fMqls(k0),1)),digits=4),round(Pco1,digits=4))
